...
async function getCachedCookies(domain) {
  ...
  try {
    ...
    const row = rows[0];
    // Cookie validi per max 2 ore — il .ASPXAUTH dell'utente dura ore/giorni
    // NON usare TTL breve: il tuo account ha full access, tmsrlmin no
    const age = Date.now() - new Date(row.updated_at).getTime();
    if (age > 120 * 60 * 1000) { console.log(`[auth] Cached cookies scaduti (>2h) domain=${domain||"wcaworld.com"}`); return null; }
...
async function ssoLogin(username, password, targetBase) {
  ...
  try {
    ...
    // Step 2: Estrai TUTTI i campi hidden dal form SSO + POST con credenziali
    ...
    // Costruisci body con TUTTI i campi hidden + credenziali
