...
async function getCachedCookies(domain) {
  ...
  try {
    ...
    const row = rows[0];
    // Cookie validi per max 15 minuti — rinnovo proattivo prima che WCA scada silenziosamente
    const age = Date.now() - new Date(row.updated_at).getTime();
    if (age > 15 * 60 * 1000) { console.log(`[auth] Cached cookies scaduti (>15min) domain=${domain||"wcaworld.com"}`); return null; }
